...
if "deleted_titles_teacher" not in st.session_state:
    st.session_state.deleted_titles_teacher = []  # ここはセッション管理用（暫定的）
...
def show_title_list():
    ...
    # ここで、教師側で削除済みタイトル（教師削除システムメッセージが存在するタイトル）を抽出
    ...
    # 学生側削除のシステムメッセージで登録されたタイトルを除外（学生側削除は教師側では無視する場合もありますが、仕様に合わせて）
    ...
    for doc in docs:
        ...
        # 既に取得済み or 学生削除されているタイトルは除外
        if title in seen_titles or title in deleted_system_titles:
            continue
        seen_titles.add(title)
        title_info[title] = {"title": title, "poster": poster}
        distinct_titles.append({"title": title, "poster": poster})
    
    # さらに、教師側で削除済みタイトル（teacher_deleted_titles）も除外する
    distinct_titles = [item for item in distinct_titles if item["title"] not in teacher_deleted_titles]
        ...
        if delete_submit:
            ...
            if docs:
                ...
                if delete_auth_key == stored_auth_key:
                    st.session_state.pending_delete_title = None
                    # ここ、教師側削除の場合、FireStore にシステムメッセージを追加し、なおかつ削除済みとしてフィルタ対象にする
                    ...
                    db.collection("questions").add({
                        ...
                        "deleted": 0,
                        "image": None,
                        "poster": title_info.get(title, {}).get("poster", "匿名")
                    })
                    st.success("タイトルを削除しました。")
                    # 両側で削除されていた場合は、完全にDBから削除
                    student_msgs = list(
                        db.collection("questions")
                        .where("title", "==", title)
