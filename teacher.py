...
def show_title_list():
    ...
    # 最も古い (orig_timestamp) を固定情報として保持し、更新日時 (update) は最新に更新
    title_info = {}
    for doc in docs:
        data = doc.to_dict()
        # システムメッセージは除外
    ...
    if not distinct_titles:
        ...
        # カラム比率 [8,2]：タイトル（投稿者名・認証コードは固定）と削除ボタン
        for idx, item in enumerate(distinct_titles):
            ...
            cols = st.columns([8,2])
            # 生徒側では認証コードは表示しないので、ラベルには投稿者のみ表示
            label = f"{title}\n(投稿者: {poster})\n最終更新: {update_time}"
    ...
    if st.session_state.pending_delete_title:
        ...
        with st.form("delete_title_form"):
            delete_auth_key = st.text_input("認証キーを入力", type="password")
