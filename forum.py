...
def show_title_list():
    ...
    # 検索：キーワードをスペースで分割し、タイトルおよび投稿者名を対象
    ...
    # 生徒側の削除システムメッセージ（"[SYSTEM]生徒はこの質問フォームを削除しました"）のタイトルを除外
    ...
    # 検索フィルタ：タイトル or 投稿者名に全キーワードが含まれるか
    if keywords:
        def match(item):
...
def show_chat_thread():
    ...
    for doc in records:
        ...
        # 修正：教師側の表示では、教師の投稿は右側・緑色、生徒の投稿は左側・白色で表示
